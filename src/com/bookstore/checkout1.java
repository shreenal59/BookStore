...
public class checkout1 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		java.util.Date ex_Date = null;
		try {
			ex_Date = new SimpleDateFormat("yyyy-MM-dd").parse(exDate);
		} catch (ParseException e1) {
			e1.printStackTrace();
		}
		java.sql.Date sqlExDate = new Date(ex_Date.getTime());
		long millis=System.currentTimeMillis();
		Date today = new java.sql.Date(millis);
		PrintWriter out = response.getWriter();
		boolean flag = true;
		if(today.compareTo(sqlExDate) > 0) {
			out.println("<script>");
            out.println("alert('Card is Exipired, change to another Card');");
            out.println("</script>");
            flag = false;
